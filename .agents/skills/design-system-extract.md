...
Extract design tokens (color, typography, spacing, radii, shadow) from a brand reference, codebase, or screenshots, and emit project-native token files. Use this when starting design work that should match an existing visual language.
...
## Phase 3: Emit the tokens

Write tokens in the format the child project already uses. In this repo that usually means `src/colors_and_type.css` for CSS custom properties and, when runtime palette switching is needed, a JS theme module such as `src/theme/palette.js`. If the source project uses another format (`tokens.ts`, `tokens.json`, Tailwind config), match it. CSS structure:
...
The output of this skill is project-native tokens plus a clear-eyed report of where the source design system has gaps or inconsistencies.
