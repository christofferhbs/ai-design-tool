...
## Phase 5: Present in one project canvas

Use the project's `ProjectShell`, `DCSection`, and `DCArtboard` components for static variations side-by-side, or use **tweaks** (see `make-tweakable`) if the variations share most of the structure and differ on a few axes. Import these primitives from `../../../src/design-tool/index.jsx`.

For a flow or multi-screen variation, build each variation as a small storyboard within the canvas.

**Do not produce v1.html / v2.html / v3.html.** One child project with all variations visible (or toggle-able) is far more useful for comparison than three separate files.
