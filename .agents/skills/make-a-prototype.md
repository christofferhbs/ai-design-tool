...
- **Device frame.** Desktop browser? iOS frame? Android frame? Macos window? Use or create the appropriate React frame component.
...
Capture this as a concise source comment near the prototype component or as a short planning note before implementation.
...
- **iOS / Android / macOS / browser** — create or reuse a React frame component in the child project.
- Keep the frame fixed; the prototype lives inside it.
- Render framed prototypes inside `DCArtboard` elements within the project's shared `ProjectShell`.
...
- **A canvas** — multiple variants side-by-side via `ProjectShell` with `DCSection`/`DCArtboard`
- **Toggles in the prototype** — the user clicks between options

Don't scatter v1.html / v2.html / v3.html across the project. One project canvas, many variants.
