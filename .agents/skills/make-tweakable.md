...
**One project canvas, many variants.** Tweaks let a single child project hold multiple visual exploration paths without scattering v1.html / v2.html / v3.html across the project.
...
## Phase 4: Use the project tweak protocol

This repo provides `useTweaks`, `TweaksPanel`, `TweakSection`, and `TweakRow` from the shared design-tool runtime:

```js
import { useTweaks, TweaksPanel, TweakSection, TweakRow } from '../../../src/design-tool/index.jsx';
```

Use those helpers instead of reimplementing the protocol or copying project-local tweak code.

The host environment exposes a toolbar toggle to show/hide the tweak panel. The helper wires up this protocol:
...
Wrap tweakable defaults in comment markers in the project entry or owning prototype module so the host can rewrite them:
...
The block between markers **must be valid JSON** (double-quoted keys and strings). In the current React projects this block normally lives in `src/app.jsx` as `TWEAK_DEFAULTS`. The host merges your edits into this block and writes the file back, so changes survive reload.
