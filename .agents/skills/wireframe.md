...
For a single screen, lay out the variations side-by-side using the project's `ProjectShell`, `DCSection`, and `DCArtboard` components. Import these from `../../../src/design-tool/index.jsx`. For a multi-screen flow, build each variation as a small storyboard (3–5 screens stacked or arranged horizontally).
