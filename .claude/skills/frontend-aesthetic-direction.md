...
Vocalize your decisions as a concise comment block near the relevant React component or project entry, and expose a rendered summary only when it helps the user compare options. **Be specific.** Vague aesthetic statements ("modern and clean") produce generic designs.
...
Write the chosen direction into the project in a durable place: a source comment near the prototype, a token file, or a concise rendered summary artboard when the user needs to compare directions. Like a junior designer showing their thinking to their manager.
