...
Use the project's existing `DesignCanvas`, `DCSection`, and `DCArtboard` components for static variations side-by-side, or use **tweaks** (see `make-tweakable`) if the variations share most of the structure and differ on a few axes.
