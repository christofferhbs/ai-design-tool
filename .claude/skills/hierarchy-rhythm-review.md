...
1. The React component, CSS file, artboard, or child project the user just edited or asked about.
2. The most recently modified design surface in the session.
3. If unclear, ask.

Read the relevant source and the styles it references. Note the medium (slide / page / mobile / dashboard) — hierarchy and rhythm rules vary by context.

## Phase 2: Run two review passes

Run both passes against the selected design surface, collecting findings as you go before fixing. Use parallel reviewers only if your environment provides them; otherwise run sequentially.
