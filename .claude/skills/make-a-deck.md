# Make a Deck: Slide Presentation in React

Build a slide presentation inside a Vite/React child project with fixed-size slides that letterbox to any viewport. Use this when the user asks for a deck, presentation, slides, or pitch.

A deck is fixed-size content (typically 1920×1080, 16:9) that scales to fit any screen. Use or create a project-local deck shell component instead of scattering slide files.
...
Before building any slide, commit to a layout system. Capture this as a concise source comment or setup component so the structure is clear.
...
Create or reuse a project-local deck shell component. It must handle:
...
- Print-to-PDF
- slide labels and validation hooks when the host uses them

Each slide is a direct child of the deck stage component.

Add labels to each slide so the user can reference them by name when commenting:

```jsx
<DeckStage>
  <Slide label="01 Title">...</Slide>
  <Slide label="02 Agenda">...</Slide>
</DeckStage>
```
...
Build one slide at a time, in order. Surface the child project path or preview as soon as you have 1–2 slides — don't perfect 15 slides in private and then reveal.
...
- Add a project-local notes data structure, one entry per slide
...
The deck shell should render speaker notes when present.
...
For end-of-turn delivery, surface the child project path and run a verification pass — build with Vite, render it headlessly when possible, and check scaling, layout, and overflow.
