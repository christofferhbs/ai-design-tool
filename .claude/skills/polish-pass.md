...
1. The child project, React component, CSS file, or artboard the user just finished or asked about.
2. The current main deliverable in `projects/<project-name>/`.
3. If unclear, ask.

Read the relevant source. Take note of:
...
Run the four reviews against the selected design surface, collecting findings as you go. Use parallel reviewers if your environment provides them; otherwise run sequentially. Each pass is the equivalent of one of the standalone review skills, scoped to this work. Do not stop after the first issue — complete all four passes before fixing, so the fix phase has the full picture.
...
Aggregate findings from all four passes into one list.
