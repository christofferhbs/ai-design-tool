...
You operate within an AI design tool repository. Design deliverables live under `projects/<project-name>/`; Claude prompt and skill procedures live in `.claude/`.
...
3. **Pick the repo-native format.** Use `DesignCanvas` with `DCSection` and `DCArtboard` for side-by-side explorations, React state for interactive prototypes, and `useTweaks` / `TweaksPanel` for live options.
...
6. **Iterate and verify.** Use your tools (shell, file reads, Vite build, browser preview, Playwright, verifier subagent if available, or equivalent) to check that designs render cleanly and behave correctly. Keep the conversation tight: report findings as a short list, not a play-by-play of every check.
...
- New visual variants go in `src/prototypes/` and are assembled in `src/app.jsx`.
...
Static variations belong on `DesignCanvas` as labeled `DCArtboard` children. A single canvas with comparable artboards beats scattered files because it lets the user judge hierarchy, rhythm, and tone side by side.
...
- **Purely visual exploration** (color, type, static layout of one element) → `DesignCanvas` with labeled `DCArtboard` cells
...
Run the relevant Vite build from the root and render the child project in a browser or headless browser when possible. If the environment provides a verifier subagent, use it for screenshot, layout, and JS probing checks; otherwise verify directly. Report findings concisely.
...
You have a library of skills in `.claude/skills/` — each is a phased procedure with explicit checks and fixes. When a user request matches a skill description, invoke the skill if your environment supports it; otherwise read that file (e.g. `.claude/skills/make-a-deck.md`) and follow it.
...
- **`polish-pass`** — End-of-design quality gate. Runs accessibility-audit, ai-slop-check, interaction-states-pass, and hierarchy-rhythm-review in parallel when supported, or sequentially when not, then fixes issues.
