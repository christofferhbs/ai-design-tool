...
import { hashFor, uniqueRouteSlug, useCanvasFocusRoute } from './route.jsx';
...
if (typeof document !== 'undefined' && !document.getElementById('dc-styles')) {
  ...
  s.textContent = [
    ...
    // Sidebar nav
    '.dc-nav{position:fixed;top:16px;left:16px;z-index:100;font-family:inherit}',
    '.dc-nav-panel{width:200px;background:#fff;border-radius:8px;overflow:hidden;',
    '  box-shadow:0 2px 8px rgba(0,0,0,.10),0 0 0 1px rgba(0,0,0,.07)}',
    '.dc-nav-head{display:flex;align-items:center;padding:4px 4px 4px 12px;',
    '  border-bottom:1px solid rgba(0,0,0,.07);min-height:38px;gap:4px}',
    '.dc-nav-back{flex:1 1 auto;display:flex;align-items:center;gap:5px;padding:4px 0;',
    '  font:500 12px/1 -apple-system,BlinkMacSystemFont,"Segoe UI",system-ui,sans-serif;',
    '  color:rgba(60,50,40,.6);text-decoration:none;border-radius:5px;transition:color .12s}',
    '.dc-nav-back:hover{color:rgba(40,30,20,.9)}',
    '.dc-nav-close{width:26px;height:26px;border:none;background:transparent;cursor:pointer;',
    '  color:rgba(60,50,40,.45);display:flex;align-items:center;justify-content:center;',
    '  border-radius:6px;transition:background .12s,color .12s;padding:0;flex-shrink:0}',
    '.dc-nav-close:hover{background:rgba(0,0,0,.06);color:rgba(40,30,20,.85)}',
    '.dc-nav-sections{padding:4px 0}',
    '.dc-nav-item{display:block;width:100%;padding:7px 14px;border:none;background:none;',
    '  text-align:left;cursor:pointer;',
    '  font:500 13px/1.3 -apple-system,BlinkMacSystemFont,"Segoe UI",system-ui,sans-serif;',
    '  color:rgba(40,30,20,.75);transition:background .1s,color .1s}',
    '.dc-nav-item:hover{background:#f0eee9;color:rgba(40,30,20,1)}',
    '.dc-nav-toggle{width:34px;height:34px;border:none;background:#fff;border-radius:8px;',
    '  cursor:pointer;display:flex;align-items:center;justify-content:center;',
    '  color:rgba(60,50,40,.6);',
    '  box-shadow:0 2px 8px rgba(0,0,0,.10),0 0 0 1px rgba(0,0,0,.07);',
    '  transition:background .12s,color .12s;padding:0}',
    '.dc-nav-toggle:hover{background:#f0eee9;color:rgba(40,30,20,.9)}',
  ].join('\n');
...
function DesignCanvas({ children, minScale, maxScale, style }) {
  ...
  const routeRegistryRef = React.useRef({});
  ...
  React.useEffect(() => {
    ...
    fetch('./' + DC_STATE_FILE)
      ...
      .then((saved) => {
        if (cancelled || !saved?.sections) return;
        skipNextWrite.current = true;
        setState((s) => ({ ...s, sections: saved.sections }));
      })
      .catch(() => {})
      .finally(() => { didRead.current = true; if (!cancelled) setReady(true); });
    const t = setTimeout(() => { if (!cancelled) setReady(true); }, 500);
  ...
  React.useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      fetch('/__canvas/write', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ path: dir + DC_STATE_FILE, content: JSON.stringify({ sections: state.sections }) }),
      }).catch(() => {});
    }, 250);
  ...
  const routeRegistry = {}; // routeSlug/routeSlug -> slotId
  const sectionMeta = {};   // sectionId -> { title, subtitle, routeSlug, slotIds[], artboardRouteSlugs }
  const sectionOrder = [];
  const sourceSections = [];
  const usedSectionSlugs = new Map();
  dcFlatten(children).forEach((sec) => {
    ...
    sourceSections.push({
      id: sid,
      title: sec.props.title ?? sid,
      subtitle: sec.props.subtitle,
      gap: sec.props.gap,
      children: sec.props.children,
    });
    sectionOrder.push(sid);
    const persisted = state.sections[sid] || {};
    const sectionTitle = persisted.title ?? sec.props.title ?? sid;
    const sectionRouteSlug = uniqueRouteSlug(sectionTitle, 'section', usedSectionSlugs);
    const abs = [];
    ...
    const srcIds = [];
    const artboardRouteSlugs = {};
    const usedArtboardSlugs = new Map();
    abs.forEach(([aid, ab]) => {
      if (hidden.includes(aid)) return;
      const slotId = `${sid}/${aid}`;
      const label = (persisted.labels || {})[aid] ?? ab.props.label ?? aid;
      const artboardRouteSlug = uniqueRouteSlug(label, 'artboard', usedArtboardSlugs);
      registry[slotId] = { sectionId: sid, artboard: ab };
      routeRegistry[`${sectionRouteSlug}/${artboardRouteSlug}`] = slotId;
      artboardRouteSlugs[aid] = artboardRouteSlug;
      srcIds.push(aid);
    });
  ...
  const syncFocusSlot = React.useCallback((slotId) => {
    setState((s) => (s.focus === slotId ? s : { ...s, focus: slotId }));
  }, []);

  React.useEffect(() => {
    routeRegistryRef.current = routeRegistry;
  });

  useCanvasFocusRoute(routeRegistryRef, syncFocusSlot);
  ...
  return (
    <DCCtx.Provider value={api}>
      <DCViewport minScale={minScale} maxScale={maxScale} style={style} panToRef={panToRef}>
        {ready && sourceSections.map((section) => (
          <DCSectionFrame
            key={section.id}
            id={section.id}
            title={section.title}
      ...
      <DCSidebar
        sections={sectionOrder.map(id => ({ id, title: sectionMeta[id]?.title ?? id }))}
        panTo={(id) => panToRef.current?.(id)}
      />
...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {}, panToRef }) {
  ...
  React.useEffect(() => {
    if (!panToRef) return;
    panToRef.current = (sectionId) => {
      const vp = vpRef.current;
      const world = worldRef.current;
      if (!vp || !world) return;
      const esc = String(sectionId).replace(/["\\]/g, '\\$&');
      const el = vp.querySelector(`[data-dc-section="${esc}"]`);
      if (!el) return;
      const vpR = vp.getBoundingClientRect();
      const elR = el.getBoundingClientRect();
      tf.current.x += (vpR.left + vpR.width / 2) - (elR.left + elR.width / 2);
      tf.current.y += (vpR.top + vpR.height / 2) - (elR.top + elR.height / 2);
      world.style.transition = 'transform 0.4s cubic-bezier(0.2,0.7,0.3,1)';
      apply();
      setTimeout(() => { world.style.transition = ''; }, 400);
    };
  }, []); // refs and apply are stable
...
// ─────────────────────────────────────────────────────────────
// DCSectionFrame — editable title + h-row of artboards in persisted order
// ─────────────────────────────────────────────────────────────
function DCSectionFrame({ id, title, subtitle, children, gap = 48 }) {
      ...
      <div style={{ display: 'flex', gap, padding: '0 60px', alignItems: 'flex-start', width: 'max-content' }}>
        {order.map((k) => (
          <DCArtboardFrame key={k} sectionId={sid} artboard={byId[k]} order={order}
            ...
            onExpand={byId[k].props.onExpand} />
        ))}
      </div>
      {rest}
    </div>
...
function DCArtboardFrame({ sectionId, artboard, label, order, onRename, onReorder, onFocus, onDelete, onExpand }) {
    ...
    <div ref={ref} data-dc-slot={id} style={{ position: 'relative', flexShrink: 0 }}>
      <div className="dc-header" data-noncommentable="" style={{ color: DC.label }} onPointerDown={(e) => e.stopPropagation()}>
        ...
        <div className="dc-btns">
          ...
          <button className="dc-expand" onClick={onExpand ?? onFocus} title="Focus">
...
function DCFocusOverlay({ entry, sectionMeta, sectionOrder }) {
  ...
  const artboardRouteSlug = meta.artboardRouteSlugs[aid];

  React.useEffect(() => {
    history.replaceState(null, '', hashFor(meta.routeSlug, artboardRouteSlug));
  }, [meta.routeSlug, artboardRouteSlug]);
...
// ─────────────────────────────────────────────────────────────
// DCSidebar — collapsible section nav + back-to-projects link.
// position:fixed so it's unaffected by canvas zoom/pan.
// ─────────────────────────────────────────────────────────────
function DCSidebar({ sections, panTo }) {
  ...
  if (!open) {
    return (
      <div className="dc-nav">
        <button className="dc-nav-toggle" onClick={() => setOpen(true)} title="Navigation">
          <svg width="14" height="11" viewBox="0 0 14 11" fill="currentColor">
            <rect x="0" y="0"   width="14" height="1.5" rx="0.75" />
            <rect x="0" y="4.75" width="14" height="1.5" rx="0.75" />
  ...
  return (
    <div className="dc-nav">
      <div className="dc-nav-panel">
        <div className="dc-nav-head">
          <a href={import.meta.env.BASE_URL} className="dc-nav-back">
            <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
              <path d="M7.5 2L3 6l4.5 4" />
            </svg>
            Projekter
          </a>
          <button className="dc-nav-close" onClick={() => setOpen(false)} title="Luk">
            <svg width="11" height="11" viewBox="0 0 11 11" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round">
              <path d="M1 1l9 9M10 1l-9 9" />
            </svg>
          </button>
        </div>
        <div className="dc-nav-sections">
          {sections.map(({ id, title }) => (
            <button key={id} className="dc-nav-item" onClick={() => panTo(id)}>
              {title}
            </button>
          ))}
        </div>
...
export { DesignCanvas, DCSection, DCArtboard, dcFlatten };
